...
**All 18 components have real implementations** — not stubs, not mocks pretending to be logic. 534 tests pass; 13 skip cleanly where a live Postgres/Redis isn't running (see [Running it](#running-it)).

**1 real decision is still open**: which broker/aggregator API backs `connect_portfolio`/`import_holdings`/`import_transactions` — a genuine external-credential gap this project's own process (see [How this was built](#how-this-was-built)) refuses to decide or set up by fiat. It ships with a working, honestly-labeled placeholder behind an injectable interface, so nothing downstream is blocked — but it isn't safe to treat as production behavior until resolved, and a real, non-broker manual-entry path exists as an alternative today ([0044](adr/0044-user-portfolio-manual-stock-entry.md)):
...
src/
  ...
  resend_client.py              Real notification delivery provider, ADR-0048
tests/                         534 real tests, one file per component
adr/                           48 Architecture Decision Records — every real decision, with
...
Set `OPENROUTER_API_KEY` (environment variable or a `.env` file at the repo root) to turn on real reasoning for Agent Runtime and Analysis & Reasoning — see [ADR-0043](adr/0043-llm-provider-resolved-openrouter.md). Set `ALPHA_VANTAGE_API_KEY` the same way to turn on real market-data/news/earnings fetching in Data & Sources — see [ADR-0046](adr/0046-data-sources-alpha-vantage-partial-resolution.md). Set `TAVILY_API_KEY` to turn on real corrective-retrieval search in Retrieval & Context — see [ADR-0047](adr/0047-retrieval-tavily-corrective-search-resolved.md). Set `RESEND_API_KEY` to turn on real email delivery in Interaction & Notification — see [ADR-0048](adr/0048-interaction-notification-resend-delivery-resolved.md) (verified live to fail safely on an unverified Resend account; add a domain at resend.com/domains for delivery to actually succeed). Set `UPSTOX_CLIENT_ID`, `UPSTOX_CLIENT_SECRET`, and `UPSTOX_REDIRECT_URI` to enable real Upstox API access for portfolio imports — the registered Upstox app redirect URI must byte-for-byte equal `UPSTOX_REDIRECT_URI`. Set `BROKER_TOKEN_ENCRYPTION_KEY` to a urlsafe-base64-encoded 32-byte Fernet key (generate one with `python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"`) to enable at-rest encryption of stored broker access tokens via `src/broker_token_crypto.py` — missing/invalid values cause the app to fail fast at startup with `BrokerConfigError`, with no plaintext fallback. Note: this project does not currently have a canonical app-secret / KMS mechanism; the env-var path is the simplest thing that works today, and should be migrated to a future KMS abstraction without changing the `encrypt_secret` / `decrypt_secret` call sites. Without any of these, each component falls back to its honest, non-cognitive/non-fetching/empty/non-sending placeholder, unchanged. Nothing else in this repo calls a real model, sends real money, or reaches a real broker API, or fetches real filings/reports/presentations/datasets; every one of those is the honestly-labeled placeholder named in the table above until you resolve its ADR.
