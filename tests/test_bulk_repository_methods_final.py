...
def test_holding_upsert_many_has_deliberate_comment():
    ...
    assert "loop over single-row upsert" in content.lower(), "Expected phrase 'loop over single-row upsert' not found in holding_repository.py"
...
def test_transaction_create_many_has_deliberate_comment():
    ...
    with open("src/repositories/transaction_repository.py", "r") as f:
        content = f.read()
    # Check for the two key phrases that indicate the deliberate comment
    assert "loop over single-row create" in content.lower(), "Expected phrase 'loop over single-row create' not found in transaction_repository.py"
